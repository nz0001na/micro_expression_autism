'''
  Step 3:
  This code is to extract optical flow feature for each video
'''
...
# compute Strain: a derivative of optical flow
...
# Step 4: extract and preprocessing
def extract_preprocess(final_images, k):
    ...
    for video in range(len(final_images)):
        ...
        for img_count in range(final_images[video].shape[0] - k):
            ...
            optical_flow = cv2.optflow.DualTVL1OpticalFlow.create()
...
if __name__ == '__main__':
    ...
    names = ['ES_190418_Female', 'EK_190514_Female', 'MK_190503_Male', 'MP_190509_Female']
    sces = [5, 6, 7, 11, 12, 13, 14]
    for sub_name in names:
        for sce in sces:
            # if sce in [13]: continue
            # if sce != 12: continue
            print(sub_name + ':' + str(sce))
            fold_name = sub_name + '_' + str(sce)
            src_path = ro + '4_cropped/'+sub_name+'/' + fold_name + '/'
            dst_path = ro + '3_OF_feature/K_17/'+sub_name+'/' + fold_name + '/'
            if os.path.exists(dst_path) is False:
                os.makedirs(dst_path)

            if os.path.exists(dst_path + fold_name + '.mat') is True: continue

            k = 17  # CASME_sq:17,  SAMMLV:159
            # dataset_name = 'ADOS'  # CASME_sq   'SAMMLV'

            final_images = load_images(src_path)
            features = extract_preprocess(final_images, k)

            data = {}
            data['fea'] = features
            sio.savemat(dst_path + fold_name + '.mat', data)
        print('done')
